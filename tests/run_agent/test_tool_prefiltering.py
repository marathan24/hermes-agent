...
_STABLE_RETRIEVAL_TOOL_NAMES = ["retrieve_tools"]
_STABLE_RETRIEVAL_VALID_NAMES = {"retrieve_tools"}
...
def test_invalid_hidden_tool_feedback_points_to_retrieve_tools(monkeypatch):
    ...
    assert "call the returned tool by its native name" in message
...
def test_retrieved_native_tool_dispatches_directly(monkeypatch):
    agent = _bare_agent(monkeypatch, "acp")
    ...
    assert set(agent._valid_tool_names_for_current_api_call()) == {
        "retrieve_tools",
        "terminal",
    }


def test_native_tool_batch_keeps_retrieved_tools_visible(monkeypatch):
    agent = _bare_agent(monkeypatch, "acp")
    ...
    assert _api_tool_names(agent) == ["retrieve_tools", "terminal"]
    assert agent._valid_tool_names_for_current_api_call() == {"retrieve_tools", "terminal"}
    assert agent._retrieved_tool_names == ["terminal"]
...
def test_unretrieved_native_tool_is_not_currently_valid(monkeypatch):
    ...
    assert "patch" not in agent._valid_tool_names_for_current_api_call()
    message = agent._invalid_tool_call_feedback(
        "patch",
        agent._valid_tool_names_for_current_api_call(),
    )
    assert "currently hidden" in message
    assert "retrieve_tools" in message
